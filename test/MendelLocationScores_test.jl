...
# @testset "initialize_optimization_location_score" begin
#     keyword1 = set_keyword_defaults!(Dict{AbstractString, Any}())
#     keyword1["flanking_distance"] = [0.5, 0.5]
#     keyword1["flanking_markers"] = 1
#     keyword1["gender_neutral"] = true
#     keyword1["lod_score_table"] = "Lod_Score_Frame.txt"
#     process_keywords!(keyword1, "location scores Control.txt", "")
...
#     parameter1 = MendelLocationScores.initialize_optimization_location_score!(
#         locus, parameter1, keyword1)

#     @test parameter1.travel == "grid"
#     @test size(parameter1.grid) == (10, 1) #(points, par)

#     # here the last entry is actually NaN in the current code because when j = 10, a = 1.0, so we 
#     # get a 0 * Inf = NaN. Pretty sure this is not expected behavior. 
#     @test all(parameter1.grid .== [-Inf; -Inf; -Inf; -Inf; -Inf; -Inf; -Inf; -Inf; -Inf; -Inf]) 
#     @test size(parameter1.par) == (1,)
#     @test parameter1.par[1] == 0.0
#     @test size(parameter1.name) == (1,)
#     @test size(parameter1.min) == (1,)
...
@testset "transmission function" begin
    ...
    for ped in 1:pedigree.pedigrees
	      for n = instruction.start[ped]:instruction.finish[ped]-1
  	        ...
  	        for l = 1:j_genotypes
  	            for k = 1:i_genotypes
  	                trans = MendelLocationScores.transmission_location_score(person, 
...
@testset "wrapper and basics" begin
    ...
    execution_error = MendelLocationScores.location_scores_option(pedigree, person, nuclear_family,
      locus, locus_frame, phenotype_frame, pedigree_frame, keyword)
    final_test = LocationScores("location scores Control.txt")

    @test execution_error == false
    @test final_test == nothing
end

# current coverage = (200, 241) ≈ 83%. This is excluding initialize_optimization_location_score 
